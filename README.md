...
- В личке пользователь может проверить статус командой `/status`.
...
- Админу доступны `/help`, `/status @username`, `/ban @username`, `/unban @username`, `/reg @username`, `/unreg @username`.
...
Директория `state/` добавлена в `.gitignore`. Если удалить `state`, бот потеряет зарегистрированных пользователей, сохраненные баны, список админов и активные challenge-проверки.
